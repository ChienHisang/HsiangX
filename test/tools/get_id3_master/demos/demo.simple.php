...
$DirectoryToScan = 'test'; // change to whatever directory you want to scan
$dir = opendir($DirectoryToScan);
echo '<table border="1" cellspacing="0" cellpadding="3">';
echo '<tr><th>Filename</th><th>Artist</th><th>Resolution</th><th>Bitrate</th><th>Playtime</th></tr>';
while (($file = readdir($dir)) !== false) {
	...
	if ((substr($file, 0, 1) != '.') && is_file($FullFileName)) {
		...
		echo '<td>'              .htmlentities(!empty($ThisFileInfo['video']['resolution_x']) && !empty($ThisFileInfo['video']['resolution_y'])  ?  $ThisFileInfo['video']['resolution_x'].'x'.$ThisFileInfo['video']['resolution_y']          : chr(160)).'</td>';
		echo '<td align="right">'.htmlentities(!empty($ThisFileInfo['bitrate'])        ?           round($ThisFileInfo['bitrate'] / 1000).' kbps' : chr(160)).'</td>';
